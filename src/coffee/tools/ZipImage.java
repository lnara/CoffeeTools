...
public class ZipImage {
	 ...
	 * @param srcFilePath
	 *            源路径
	 * @param descFilePath目标路径
	 * @deprecated 这个不太好用，太多时会内存溢出
	 */
	 ...
	 * @param descFilePath目标路径
	 * @param level
	 *            压缩程度， 取值0~1范围内， 越小 压缩 程度越高，图片越不清楚,压缩后文件体积越小
	 * @deprecated 这个不太好用，太多时会内存溢出
	 */
	public static boolean compressPic(String srcFilePath, String descFilePath, float level) {
		File file = null;
		BufferedImage src = null;
		FileOutputStream out = null;
		ImageWriter imgWrier;
		...
		imgWriteParams.setCompressionMode(ImageWriteParam.MODE_EXPLICIT);
		// 这里指定压缩的程度，参数qality是取值0~1范围内，
		imgWriteParams.setCompressionQuality(level);
		imgWriteParams.setProgressiveMode(ImageWriteParam.MODE_DISABLED);
		ColorModel colorModel = ColorModel.getRGBdefault();
		// 指定压缩时使用的色彩模式
		imgWriteParams.setDestinationType(new javax.imageio.ImageTypeSpecifier(colorModel, colorModel
